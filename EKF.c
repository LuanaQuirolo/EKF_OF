...
void ofs_ekf_init(ofs_ekf_t* filtro, double m[N_MAG], double a[N_IMU], double z){
    ...
    mulmat(N_PROC_NOISE, N_PROC_NOISE, N_STATES, Q, Wt, filtro->aux4); // aux4 = Q * Wt
    mulmat(N_STATES, N_PROC_NOISE, N_STATES, W, filtro->aux4, filtro->WQWt); //  W * Q * Wt
...
void prediction_step(ofs_ekf_t* filtro, mediciones_t *u){ 
    ...
    add(N_STATES, N_STATES, filtro->WQWt, filtro->aux7, filtro->cov); // cov = F * cov * Ft +  W * Q * Wt
